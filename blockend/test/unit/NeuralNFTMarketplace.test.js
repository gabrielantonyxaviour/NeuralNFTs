  ...
  : describe("NeuralNFT Marketplace Unit Tests", function () {
      ...
      describe("mintNft", function () {
        ...
        it("sets token royalty and verifies royalty address", async function () {
          const [royaltyAddress, royaltyAmount] =
            await nftMarketplace.royaltyInfo(TOKEN_ID, PRICE);
        ...
        it("emits event for minting NFT", async function () {
          expect(
            await nftMarketplace.mintNft(TOKEN_URI_2, user.address, 1000),
        ...
        it("reverts if royalty is too high", async function () {
          ...
          await expect(
            ...
            `NeuralNFTMarketplace__RoyaltyFeesTooHigh(${royalty_fees})`,
          );
        });
      });
      ...
      describe("setRoyaltyInfo", function () {
        ...
        it("sets token royalty and verifies royalty address", async function () {
          await nftMarketplace.setRoyaltyInfo(
            TOKEN_ID,
            accounts[4].address,
            690,
          );
          ...
          assert.equal(
            ...
            parseInt(PRICE * royaltyFraction) - 1,
          );
      ...
      describe("listItem", function () {
        ...
        it("exclusively allows owners to list", async function () {
          ...
          await expect(
            nftMarketplace.listItem(TOKEN_ID, PRICE),
      ...
      describe("cancelListing", function () {
        it("reverts if there is no listing", async function () {
          await expect(
            nftMarketplace.cancelListing(TOKEN_ID),
          ).to.be.revertedWith(`NotListed(${TOKEN_ID})`);
        });
        ...
        it("emits event and removes listing", async function () {
          await nftMarketplace.listItem(TOKEN_ID, PRICE, { value: LIST_FEE });
          expect(await nftMarketplace.cancelListing(TOKEN_ID)).to.emit(
            "ItemCanceled",
          );
          const listing = await nftMarketplace.getListing(TOKEN_ID);
      ...
      describe("buyItem", function () {
        ...
        it("transfers the nft to the buyer, adds earnings to seller and sends platform fee to owner", async function () {
          ...
          let platformFeePercentage =
            (await nftMarketplace.getPlatformFee()) * 0.01;
          nftMarketplace = nftMarketplaceContract.connect(user);
          await nftMarketplace.listItem(TOKEN_ID, PRICE, { value: LIST_FEE });
          nftMarketplace = nftMarketplaceContract.connect(accounts[3]);

          expect(
            await nftMarketplace.buyItem(TOKEN_ID, { value: userEthPaid }),
          ).to.emit("ItemBought");
          ...
          // check ownership
          assert.equal(
            await nftMarketplace.ownerOf(TOKEN_ID),
            accounts[3].address,
          );
      ...
      describe("updateListing", function () {
        ...
        it("reverts if list fee is insufficient", async function () {
          await nftMarketplace.listItem(TOKEN_ID, PRICE, { value: LIST_FEE });
          await expect(
            nftMarketplace.updateListing(TOKEN_ID, PRICE, {
              value: LIST_FEE - 100,
            }),
          ).to.be.revertedWith("NeuralNFTMarketplace__InsufficientFunds");
        });
        ...
        it("Sends list fee to the owner", async function () {
          await nftMarketplace.listItem(TOKEN_ID, PRICE, { value: LIST_FEE });
          await nftMarketplace.updateListing(TOKEN_ID, PRICE + 2000, {
            value: LIST_FEE,
          });
          nftMarketplace = nftMarketplaceContract.connect(deployer);
          let ethEarnedByUser = await nftMarketplace.getEarnings();
      ...
      describe("setListFee", function () {
        it("exclusive only to the owner", async function () {
          ...
          await expect(
            nftMarketplace.setListFee(LIST_FEE + 100),
          ).to.be.revertedWith("NeuralNFTMarketplace__NotOwner");
        });
