...
export class ApprovedForAddress extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save ApprovedForAddress entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type ApprovedForAddress must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("ApprovedForAddress", id.toString(), this);
    }
  }

  static load(id: string): ApprovedForAddress | null {
    return changetype<ApprovedForAddress | null>(
      store.get("ApprovedForAddress", id)
    );
  }
