...
contract NeuralNFTs is Ownable {
  INeuralNFTsNFT NeuralNFTsNFT;
  INeuralNFTsMarket NeuralNFTsMarket;

  constructor(address nftContract, address marketContract) {
    NeuralNFTsNFT = INeuralNFTsNFT(nftContract);
    NeuralNFTsMarket = INeuralNFTsMarket(marketContract);
  }
  ...
  function createProposal(uint256 listingId, bool buy) public {
    ...
    if (buy) {
      ...
      _daoProposal.listing.nftTokenId = NeuralNFTsMarket.getNftId(listingId);
      _daoProposal.listing.daoID = NeuralNFTsMarket.getDAOID(listingId);
  ...
  function executeProposal() public {
    ...
    if (_daoProposal.proposalType == ProposalType.BUY) {
      if (_daoProposal.acceptVotes > _daoProposal.rejectVotes) {
        ...
        if (NeuralNFTsMarket.checkListingExpiry(_daoProposal.listing.listingId)) {
          //Swapping in NFT contract
          NeuralNFTsNFT.nftSwapOwnership(
          ...
          NeuralNFTsMarket.expireListing(_daoProposal.listing.listingId);
        } else {
      ...
      NeuralNFTsMarket.createListing(_dao.NFTId, userToDAO[msg.sender]);
    }
  ...
  function onERC1155Received(
    ...
    uint256 _daoID = NeuralNFTsNFT.nftOwnershipCheck(tokenId);
